...
<div class="modal fade" id="deleterecord" role="dialog">
  <div class="modal-dialog modal-sm" role="content">
    ...
    <div class="access_button" align="center" style="margin-top:50px;">
      <button type="submit" class="btn btn-success" name="add" id="openstudent" data-toggle="modal" data-target="#addstudents">ADD</button>
      <button type="submit" class="btn btn-warning" name="update" id="openupdate"data-toggle="modal" data-target="#updatemarks">UPDATE</button>
      <button type="submit" class="btn btn-danger" name="delete"data-toggle="modal" data-target="#deleterecord">DELETE</button>
    </div>
...
if($php1!=0 && $mysql1!=0 && $html1!=0){
  $sqlcom="INSERT INTO `student_data`( `Firstname`, `Lastname`, `Email`, `Userid`, `Password`, `PHP`, `HTML`, `MYSQL`, `Total`, `Percentage`) VALUES ('$fname1','$lname1','$email1','$uname1','$pname1','$php1','$html1','$mysql1','$total','$per') ";
...
if(isset($_POST['update1'])){
  ...
  $sqlcom1="UPDATE `student_data` SET `PHP`='$upphp',`HTML`='$uphtml',`MYSQL`='$upmysql',`Total`='$total1',`Percentage`='$per1' WHERE `id`='$num'";
...
if(isset($_POST['delete1'])){
  $num1=$_POST['idno'];
  if ($num1!=0) {
    $sqlcom2="DELETE FROM `student_data` WHERE `id`='$num1'";
